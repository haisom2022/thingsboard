...
import { EnduserComponent } from '@modules/home/pages/enduser/enduser.component';
...
import { AddEnduserDialogComponent, AddUserDialogData } from '@modules/home/pages/enduser/add-enduser-dialog.component';
...
import { EnduserTabsComponent } from '@home/pages/enduser/enduser-tabs.component';
...
@Injectable()
export class EndusersTableConfigResolver implements Resolve<EntityTableConfig<User>> {
  ...
  constructor(private store: Store<AppState>,
    ...
    this.config.entityComponent = EnduserComponent;
    this.config.entityTabsComponent = EnduserTabsComponent;
    ...
    this.config.deleteEntity = id => this.userService.deleteEnduser(id.id);
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<EntityTableConfig<User>> {
    ...
    return this.store.pipe(select(selectAuth), take(1)).pipe(
      tap((auth) => {
        ...
        this.authority = auth.userDetails.authority;
        //this.tenantId = routeParams.tenantId;
        this.tenantId = auth.userDetails.tenantId.id;
        //this.customerId = NULL_UUID;
        this.customerId = auth.userDetails.customerId.id;
        if (this.authority === Authority.TENANT_ADMIN) {
          this.config.entitiesFetchFunction = pageLink => this.userService.getEndUsersByTenantId(this.tenantId, pageLink);
        } else {
          this.config.entitiesFetchFunction = pageLink => this.userService.getEndUsersByCustomerId(this.customerId, pageLink);
        }
  ...
  addUser(): Observable<User> {
    return this.dialog.open<AddEnduserDialogComponent, AddUserDialogData,
      User>(AddEnduserDialogComponent, {
      disableClose: true,
