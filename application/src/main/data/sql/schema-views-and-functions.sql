...
SELECT d.*
       ...
       , COALESCE((c.additional_info::json->>'isPublic')::bool, FALSE) as customer_is_public
        ,u.email AS enduser_email
        ,u.first_name AS enduser_first_name
        ,u.last_name AS enduser_last_name
        ,u.phone AS enduser_phone
       , d.type as device_profile_name
       , COALESCE(da.bool_v, FALSE) as active
FROM device d
         LEFT JOIN customer c ON c.id = d.customer_id
         LEFT JOIN tb_user u ON u.id = d.enduser_id
